...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(
        ...
        # Run VLA inference
        # print("[DEBUG] About to call self.generate() in predict_action")
        # print("[DEBUG] Flushing stdout before generate...")
        # import sys
        # sys.stdout.flush()
        generated_ids = self.generate(input_ids, max_new_tokens=self.get_action_dim(unnorm_key), **kwargs)
        # print("[DEBUG] self.generate() completed, about to hit pdb")
        # sys.stdout.flush()
        
        # # Try multiple debugging methods
        # print("\n" + "="*80)
        # print("DEBUG: About to enter debugger - if you see this, the code is executing!")
        # print("="*80 + "\n")
        # sys.stdout.flush()
        
        # # Write to a file to confirm we reached this point
        # try:
        #     with open("/tmp/openvla_debug_reached.txt", "w") as f:
        #         f.write(f"Reached pdb line at {time.time()}\n")
        #         f.write(f"generated_ids shape: {generated_ids.shape}\n")
        # except Exception as e:
        #     print(f"[DEBUG] Could not write debug file: {e}")
        
        # # Use breakpoint() which is more reliable in Python 3.7+
        # # If that doesn't work, try pdb.set_trace() with explicit sys.stdin
        # print(f"[DEBUG] Waiting 5 seconds before breakpoint - check /tmp/openvla_debug_reached.txt")
        # sys.stdout.flush()
        # time.sleep(5)  # Give you time to see the message
        
        # try:
        #     breakpoint()  # Python 3.7+ - uses PYTHONBREAKPOINT env var
        # except:
        #     import pdb
        #     # Force pdb to use the real stdin/stdout
