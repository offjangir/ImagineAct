...
class Diffusion(nn.Module):
    ...
    def loss_fn(self, model: nn.Module, x: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        ...
        if torch.isnan(x_t).any():
            logging.error("NaN detected in noised input x_t")
            return torch.tensor(float('nan'), device=x.device)

        pred_v = model(x_t, t, actions)
        if torch.isnan(pred_v).any():
            logging.error("NaN detected in model prediction pred_v")
            return torch.tensor(float('nan'), device=x.device)

        # Build target v
        alphas_cumprod = self.alphas_cumprod[t.reshape(-1)].view(B, T, 1, 1, 1)
        
        # Check for numerical instability in sqrt operations
        if ((1 - alphas_cumprod) < 0).any():
            logging.error("Negative value detected before sqrt in target_v calculation")
            return torch.tensor(float('nan'), device=x.device)
            
        target_v = alphas_cumprod.sqrt() * noise - (1 - alphas_cumprod).sqrt() * x
        if torch.isnan(target_v).any():
            logging.error("NaN detected in target_v")
            return torch.tensor(float('nan'), device=x.device)
