    ...
    for clip_idx, (x_gt_clip, actions_clip) in enumerate(clip_action_pairs):
        ...
        while current_frame < T:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                #Get action from VLA
                last_frame = x_context[0, -1]  # [H, W, C]
                # last_frame_np = (last_frame.float().clamp(0, 1) * 255).byte().cpu().numpy()
                # pil_image = Image.fromarray(last_frame_np)
                new_frame = last_frame.cpu().numpy()
                new_frame = np.clip(new_frame * 255, 0, 255).astype(np.uint8)
                pil_image = Image.fromarray(new_frame)
                pil_image.save(output_dir / f"debug_frame_{current_frame}.png")
                # import pdb; pdb.set_trace()
                inputs = processor(prompt, pil_image).to(device="cuda", dtype=torch.bfloat16)
                action_new = vla.predict_action(**inputs, unnorm_key=unnorm_key, do_sample=False) # [action_dim] ndarray
                print(f"[Eval] Generated action for frame {current_frame}: {action_new}")

                action_new = torch.tensor(action_new).to(device="cuda", dtype=torch.bfloat16).unsqueeze(0).unsqueeze(0)  # [1, 1, action_dim]
                action_new = torch.cat([action_new, action_new.new_zeros(1, 1, 3)], dim=-1)  # pad with zeros to match action_dim
                
                if actions_context is None:
                    actions_context = action_new.expand(-1, n_context_frames, -1)  # [1, n_context_frames, action_dim]
                else:
                    actions_context = actions_context[:, -n_context_frames:] # Slide window
                    actions_context = torch.cat([actions_context, action_new], dim=1) 
                actions_for_gen = actions_context  # [1, n_context_frames+1, action_dim]     
                
                #Encode context frames for World Model
                ...
                noise_frame = torch.randn((B, 1, H_lat, W_lat, C_lat), device=device, dtype=latents_context.dtype)
