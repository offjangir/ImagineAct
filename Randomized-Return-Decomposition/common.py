...
# Import Tester from scripts directory
...
from utils.os_utils import get_arg_parser, get_logger, str2bool, make_dir
...
def get_args():
    ...
    # Make --cuda work as both a flag (--cuda) and with value (--cuda True/False)
    def cuda_type(x):
        if x is None:
            return True  # Flag used without value
        return str2bool(x)
    parser.add_argument('--cuda', help='whether to use CUDA (GPU)', 
                       type=cuda_type, default=True, nargs='?', const=True)
    parser.add_argument('--device', help='CUDA device to use (e.g., "cuda:0", "cuda:2")', 
                       type=str, default=None)
    
    # WandB arguments
    parser.add_argument('--use_wandb', help='whether to use Weights & Biases for logging', 
                       type=str2bool, default=False)
    parser.add_argument('--wandb_project', help='WandB project name', type=str, default=None)
    parser.add_argument('--wandb_entity', help='WandB entity/username', type=str, default=None)
    parser.add_argument('--wandb_run_name', help='WandB run/experiment name (overrides tag)', type=str, default=None)
    ...
    parser.add_argument('--checkpoint_dir', help='directory to save checkpoints', type=str, default='log/checkpoints')
    parser.add_argument('--save_freq', help='save checkpoint every N epochs (0 to disable periodic saves)', type=np.int32, default=1)
    parser.add_argument('--load_path', help='path to checkpoint file to load (for resuming training or evaluation)', type=str, default=None)
    parser.add_argument('--save_final', help='whether to save final checkpoint at end of training', type=str2bool, default=True)
    ...
    def libero_args():
        parser.add_argument('--libero_dataset_path', 
                           help='Path to LIBERO RLDS dataset directory', 
                           type=str, 
                           default='/data/kmirakho/JustImagine/modified_libero_rlds/libero_10_no_noops/1.0.0')
        parser.add_argument('--libero_image_size', 
                           help='Image size for LIBERO observations (will be resized from 256x256)', 
                           type=int, 
                           default=84)
        parser.add_argument('--libero_max_trajectories', 
                           help='Maximum number of trajectories to load from dataset (None = load all)', 
                           type=int, 
                           default=None)
        # Set defaults for LIBERO (offline RL)
        parser.set_defaults(
            epochs=10,
            cycles=50,
            iterations=100,
            ...
            warmup=0,  # No warmup needed for offline RL
            buffer_size=100000,  # Smaller buffer for offline
            rrd_batch_size=32,
            rrd_sample_size=32
        )
    ...
    def basis_alg_args():
        ...
        defaul_basis_alg = {'mujoco': 'sac', 'atari': 'dqn', 'libero': 'sac'}.get(env_category, 'sac')
    ...
    def rrd_args():
        basis_alg_args()
        args, _ = parser.parse_known_args()
        default_batch_size, default_sample_size = {'mujoco': (256, 64), 'atari': (32, 32), 'libero': (32, 32)}.get(env_category, (32, 32))
        ...
        parser.add_argument('--rrd_reward_only', help='only train reward network, skip policy learning (for supervised reward learning)', type=str2bool, default=False)
        parser.add_argument('--rrd_bias_correction', help='whether to use bias-correction loss', type=str2bool, default=False)
        
        # OpenVLA feature arguments
        parser.add_argument('--use_openvla_features', help='use OpenVLA language and vision features for reward model', type=str2bool, default=False)
        parser.add_argument('--openvla_checkpoint', help='path to OpenVLA checkpoint directory', type=str, default=None)
        parser.add_argument('--openvla_task_description', help='task description for OpenVLA (for LIBERO)', type=str, default='')
        parser.add_argument('--openvla_reward_hidden_dim', help='hidden dimension for OpenVLA reward network', type=np.int32, default=512)
        parser.add_argument('--openvla_reward_num_layers', help='number of MLP layers in OpenVLA reward network', type=np.int32, default=3)
...
def experiment_setup(args):
    ...
    if args.load_path is not None and os.path.exists(args.load_path):
        args.logger.info(f'Loading checkpoint from {args.load_path}')
        try:
            args.agent.load_model(args.load_path)
            args.logger.info('*** checkpoint loaded successfully ***')
        except Exception as e:
            args.logger.warning(f'Failed to load checkpoint: {e}')
            args.logger.warning('Continuing with randomly initialized weights')
    elif args.load_path is not None:
        args.logger.warning(f'Checkpoint path specified but file not found: {args.load_path}')
        args.logger.warning('Continuing with randomly initialized weights')
